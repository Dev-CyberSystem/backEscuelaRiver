...
//   }
// };
// export const updateConvocatedStatus = async (req, res) => {
//   try {
//     const alumno = await Alumno.findById(req.params.id);
//     if (alumno) {
//       alumno.convocated = req.body.convocated;
//       await alumno.save();
//       res.json(alumno);
//     } else {
...
export const getAllAlumnos = async (req, res) => {
  try {
    ...
    const filter = categoria ? { categoria: parseInt(categoria, 10) } : {};
    const alumnos = await Alumno.find(filter).populate('pagos');
    res.status(200).json(alumnos);
  } catch (error) {
    res.status(500).json({ message: 'Error al obtener alumnos', error });
  }
...
export const getAlumnoByDni = async (req, res) => {
  ...
  try {
    ...
    if (!alumnoFound) {
      return res.status(404).json({ message: "No hemos podido encontrar el alumno solicitado" });
    }
...
export const getCategorias = async (req, res) => {
  try {
    const categorias = await Alumno.distinct('categoria');
